...
static std::string GenerateIncludeGuard(const Parser& parser, const std::string& file_name) {
...
static std::string GenerateDependenceHeaders(const Parser& parser, const std::string& file_name) {
  std::string code;
  int num_includes = 0;
  for (auto it = parser.included_files_.begin();
    ...
    if (basename != file_name) {
      code += "#include \"" + basename + "_generated.hpp\"\n";
      num_includes++;
    }
  }
...
std::string GenerateCppOne(const Parser &parser,
                           const std::string& file_name,
                           const std::string& body,
                           const std::string& forward_decl_code_same_namespace,
                           const std::string& forward_decl_code_other_namespace, 
                           bool include_dependence_headers,
                           bool generate_global_helper) {
  using namespace cpp;

  std::string code;
  code = "// automatically generated by the FlatBuffers compiler, do not modify\n\n";

  std::string include_guard = GenerateIncludeGuard(parser, file_name);
  code += "#ifndef " + include_guard + "\n";
  code += "#define " + include_guard + "\n\n";

  code += "#include \"flatbuffers/flatbuffers.h\"\n\n";
  if (include_dependence_headers) {
    code += GenerateDependenceHeaders(parser, file_name);
  }
...
static std::map<std::string, std::string> GenerateCppMap(const Parser &parser, const std::string& file_name) {
  using namespace cpp;

  std::map<std::string, std::string> enum_code_map;
  std::map<std::string, std::string> enum_code_post_map;
  std::map<std::string, std::string> struct_code_map;
  std::map<std::string, std::string> table_code_map;
  ...
  for (auto it = parser.enums_.vec.begin();
    ...
    enum_code_map[(*it)->name] = enum_code;
    enum_code_post_map[(*it)->name] = enum_code_post;
  }
  ...
  for (auto it = parser.structs_.vec.begin();
    ...
    if ((**it).fixed) {
      ...
      GenStruct(parser, **it, &struct_code);
      struct_code_map[(*it)->name] = struct_code;
    }
  }
  for (auto it = parser.structs_.vec.begin();
       it != parser.structs_.vec.end(); ++it) {
    if (!(**it).fixed) {
      ...
      GenTable(parser, **it, &table_code);
      table_code_map[(*it)->name] = table_code;
    }
  }

  // Only output file-level code if there were any declarations.
  std::string s = forward_decl_code_same_namespace, o = forward_decl_code_other_namespace;
  std::map<std::string, std::string> code_map;
  if (!parser.opts.multi_file) {
    // single file
    std::string code;
    for (auto c : enum_code_map)      code += c.second;
    for (auto c : struct_code_map)    code += c.second;
    for (auto c : table_code_map)     code += c.second;
    for (auto c : enum_code_post_map) code += c.second;
    code_map[file_name] = GenerateCppOne(parser, file_name, code, s, o, parser.opts.include_dependence_headers, parser.root_struct_def_);
  } else {
    // multi file
    for (auto c : enum_code_map) {
      if (c.second.length()) {
        code_map[c.first] += GenerateCppOne(parser, c.first, c.second, "", "", false, false);
      }
    }
    for (auto c : struct_code_map) {
      if (c.second.length()) {
        bool generate_global_helper = parser.root_struct_def_ && c.first == parser.root_struct_def_->name;
        code_map[c.first] += GenerateCppOne(parser, c.first, c.second, s, o, false, generate_global_helper);
      }
    }
    for (auto c : table_code_map) {
      if (c.second.length()) {
        bool generate_global_helper = parser.root_struct_def_ && c.first == parser.root_struct_def_->name;
        code_map[c.first] += GenerateCppOne(parser, c.first, c.second, s, o, false, generate_global_helper);
      }
    }
    // FIXME support enum post code
    for (auto c : enum_code_post_map) {
      if (c.second.length()) {
        code_map[c.first] += GenerateCppOne(parser, c.first, c.second, "", "", false, false);
      }
    }
...
static std::string GenerateRootCpp(const Parser& parser,
                                   const std::string& file_name,
  ...
  for (auto c : code_map) {
    if (c.second.length()) {
      code += "#include \"" + GeneratedFileName("", c.first) + "\"\n";
    }
  }
  code += "\n#endif  // " + include_guard + "\n";
  return code;
...
bool GenerateCPP(const Parser &parser,
                 ...
                 const std::string &file_name) {
  bool ret = true;
  auto code_map = GenerateCppMap(parser, file_name);
  std::string code;
  if (!parser.opts.multi_file) {
    // single file
    ...
    for (auto c : code_map) {
      if (c.second.length()) {
        std::string file_path = GeneratedFileName(path, c.first);
        if (!SaveFile(file_path.c_str(), c.second, false)) {
          ret = false;
        }
      }
    }
    ...
    if (!code_map.empty()) {
      code = GenerateRootCpp(parser, file_name, code_map);
    }
  }

  if (code.length()) {
    std::string file_path = GeneratedFileName(path, file_name + "_generated");
    if (!SaveFile(file_path.c_str(), code, false)) {
      ret = false;
    }
  }
  return ret;
...
std::string CPPMakeRule(const Parser &parser,
  ...
  std::string make_rule = GeneratedFileName(path, filebase + "_generated") + ": ";
