...
size_t xxtea_to_key_array(const char * key, size_t len, uint32_t* key_array, size_t key_array_size) {
    ...
    assert(key_array_size >= n * sizeof(uint32_t));

#if defined(BYTE_ORDER) && (BYTE_ORDER == LITTLE_ENDIAN)
    memcpy(key_array, fixed_key, len);
#else
    for (i = 0; i < len; ++i) {
        key_array[i >> 2] |= (uint32_t)fixed_key[i] << ((i & 3) << 3);
    }
#endif
    return n;
}
...
static size_t xxtea_to_ubyte_array(const uint32_t * data, size_t len, int inc_len, uint8_t *out, size_t out_size) {
#if !(defined(BYTE_ORDER) && (BYTE_ORDER == LITTLE_ENDIAN))
    size_t i;
#endif
    size_t m, n;
    ...
    if (inc_len) {
        m = data[len - 1];
        n -= 4;
        if ((m < n - 3) || (m > n)) return 0;
        n = m;
    }
    assert(out_size >= n + 1);

#if defined(BYTE_ORDER) && (BYTE_ORDER == LITTLE_ENDIAN)
    memcpy((void*)out, data, n);
#else
    for (i = 0; i < n; ++i) {
...
size_t xxtea_ubyte_encrypt(const uint8_t * data, size_t len, uint8_t* out, size_t out_size, const uint32_t * key_array) {
    ...
    size_t out_len = xxtea_to_ubyte_array(xxtea_uint_encrypt(data_array, data_len, key_array), data_len, 0, out, out_size);
    free(data_array);
...
size_t xxtea_ubyte_decrypt(const uint8_t * data, size_t len, uint8_t* out, size_t out_size, const uint32_t * key_array) {
    ...
    size_t out_len = xxtea_to_ubyte_array(xxtea_uint_decrypt(data_array, data_len, key_array), data_len, 1, out, out_size);
    free(data_array);
...
size_t xxtea_encrypt(const void * data, size_t len, uint8_t* out, size_t out_size, const char * key) {
    uint32_t key_array[64];
    xxtea_to_key_array(key, strlen(key), key_array, sizeof(key_array));
    return xxtea_ubyte_encrypt((const uint8_t *)data, len, out, out_size, key_array);
}

size_t xxtea_decrypt(const void * data, size_t len, uint8_t* out, size_t out_size, const char * key) {
    uint32_t key_array[64];
    xxtea_to_key_array(key, strlen(key), key_array, sizeof(key_array));
    return xxtea_ubyte_decrypt((const uint8_t *)data, len, out, out_size, key_array);
}
